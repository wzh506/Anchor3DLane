...
def unit_update_projection_extrinsic(args, extrinsics, intrinsics):
    ...
    """
    if len(extrinsics.shape) == 3: #说明是batch_size且是tensor
        ...
        if not args.no_cuda:
            M_inv = M_inv.cuda()
        return M_inv #输出是tensor
    else: #说明在batch里面
        
        M_inv = np.zeros((3, 3), dtype=np.float32)
        _M, _M_inv = homography_im2ipm_norm(args.top_view_region, np.array([args.org_h, args.org_w]),
                                            args.crop_y, np.array([args.resize_h, args.resize_w]),
                                            None, None, intrinsics, extrinsics)
        M_inv = _M_inv

        return M_inv #输出是numpy
...
def unit_update_projection_for_data_aug(args, aug_mats, _M_inv, _S_im_inv=None, _S_im=None):
    ...
    """
    if isinstance(aug_mats,torch.Tensor):
        if not args.no_cuda:
        ...
        if _S_im_inv is None:
            _S_im_inv = np.array([[1/np.float(args.resize_w),                         0, 0],
                                  [                        0, 1/np.float(args.resize_h), 0],
                                  [                        0,                         0, 1]], dtype=np.float32)
        
        if _S_im is None:
            _S_im = np.array([[args.resize_w,              0, 0],
                              [            0,  args.resize_h, 0],
                              [            0,              0, 1]], dtype=np.float32)
        ...
        # self.H_c[i] = torch.matmul(aug_mats[i], self.H_c[i])
        # augmentation need to be applied in unnormalized image coords for M_inv
        aug_mats = np.matmul(np.matmul(_S_im_inv, aug_mats), _S_im)
        _M_inv = np.matmul(aug_mats, _M_inv)
