...
class LaneVis(object):
    ...
    def vis_raw(self, gt, save_dir, img_dir, img_name, prob_th=0.5):
        img_path = os.path.join(img_dir, img_name)
        fig = plt.figure(figsize=(35,20))
        ax1 = fig.add_subplot(131)
        ax2 = fig.add_subplot(132)
    ...
    def visualize(self, pred_file, gt_file, img_dir=None, save_dir=None, prob_th=0.5, vis_step=10):
        ...
        if pred_file is not None:
            ...
            for i, pred in tqdm(enumerate(json_pred)): #全部保存吧
                # if i % vis_step != 0:
                #     continue #原来Pred跳过了一部分！
                ...
                self.vis_pred(gt,pred, save_dir, img_dir, raw_file, prob_th=prob_th) #这个函数会用到gt，但是不会绘制gt
        else:
            json_gt = [json.loads(line) for line in open(gt_file).readlines()]
            gts = {l['raw_file']: l for l in json_gt}
            for i, gt in tqdm(enumerate(json_gt)):
                # 每个都保留
                # if i % vis_step != 0:
                #     continue
                # if 'raw_file' not in pred or 'laneLines' not in pred:
                ...
                self.vis_raw(gt, save_dir, img_dir, raw_file, prob_th=prob_th)
