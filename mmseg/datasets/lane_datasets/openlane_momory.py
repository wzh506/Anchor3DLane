...
class OpenlaneMODataset(OpenlaneDataset):
    def __init__(self, 
                 pipeline,
                 data_root,
                 prev_dir=None,
        ...
        super(OpenlaneMODataset, self).__init__(pipeline, data_root, **kwargs)
    ...
    def load_annotations(self):
        ...
        for i in range(len(self.img_infos)):
            self.prev_infos[self.img_infos[i]['filename']] = i #直接找回去原来的索引,模型运行中是不会变得
        print("successful generate prev annotation")
    ...
    def __getitem__(self, idx, transform=False):
        ...
        # prev_idx = self.prev_infos[prev_images[0]]
        
        try:
            prev_idx = self.prev_infos[prev_images[0]] #倒霉的话可能不存在
        except:
            print('No prev data for {},have to generate another file'.format(results['prev_file']))
            return self.__getitem__(idx+1, transform)
        
        results_prev = self.img_infos[idx].copy()
        results_prev['img_info'] = {}
        results_prev['img_info']['filename'] = results['filename']
        results_prev['ori_filename'] = results['filename']
        results_prev['ori_shape'] = (self.h_org, self.w_org)
        results_prev['flip'] = False
        results_prev['flip_direction'] = None
        with open(results_prev['anno_file'], 'rb') as f:
            obj = pickle.load(f)
            results_prev.update(obj)
        if self.no_cls:
            results_prev['gt_3dlanes'][:, 1] = results_prev['gt_3dlanes'][:, 1] > 0
        # results_prev['img_metas'] = {'ori_shape':results_prev['ori_shape']} #额外计算一个M_inv
        # results_prev['gt_project_matrix'] = projection_g2im_extrinsic(results_prev['gt_camera_extrinsic'], results_prev['gt_camera_intrinsic'])
        # results_prev['gt_homography_matrix'] = homography_g2im_extrinsic(results_prev['gt_camera_extrinsic'], results_prev['gt_camera_intrinsic'])

        aug_mat = np.eye(3) #没有使用数据增强，直接传入单位矩阵；直接把自己这个类送进去
        M_inv = unit_update_projection_extrinsic(self, results_prev['gt_camera_extrinsic'], results_prev['gt_camera_intrinsic'])
        M_inv = unit_update_projection_for_data_aug(self, aug_mat, M_inv)
        results_prev['M_inv'] = M_inv
        results['M_inv']=np.stack([results['M_inv'],results_prev['M_inv']],axis=0)
